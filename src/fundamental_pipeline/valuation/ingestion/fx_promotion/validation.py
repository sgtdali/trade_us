...
FX-close promotion bridge.

Structurally parallel to ``ingestion.promotion.validation``. Every
function here is pure: no file I/O, no process clock, no pair literal,
and no network.
"""
...
from .models import SelectedFxRow


def check_bundle_domain_eligibility(bundle_manifest: Mapping[str, Any], *, pair_id: str) -> list[Finding]:
    """Domain-level (not structural -- structural integrity is proven by
    the caller via ``ingestion.fx.service.validate_fx_bundle``/
    ``replay_fx_bundle`` before this is ever called) checks on the source
    ingestion bundle's own recorded outcome."""
    findings: list[Finding] = []
    requested = bundle_manifest.get("requested_pairs") or []
    if pair_id not in requested:
        findings.append(Finding(
            rule_id="FXPROMO-BUNDLE-001", severity="blocker", scope="bundle",
            reason_code="identity.pair_not_requested",
            message=f"pair {pair_id!r} was not among the bundle's requested_pairs {requested!r}",
        ))
    if bundle_manifest.get("overall_status") != "valid":
        findings.append(Finding(
            rule_id="FXPROMO-BUNDLE-002", severity="blocker", scope="bundle",
            reason_code="identity.overall_status_invalid",
            message=f"source FX ingestion bundle overall_status is {bundle_manifest.get('overall_status')!r}, not 'valid'",
        ))
    return findings


def check_provider_manifest_compatibility(provider_extract: Mapping[str, Any], manifest: Mapping[str, Any]) -> list[Finding]:
    """Exact-compatibility checks between the source FX provider-extract
    and the target market-source-manifest. The manifest must name the
    same fixed provider the bundle was actually fetched from."""
    findings: list[Finding] = []
    ...
    if provider.get("provider_id") != authority.get("provider_id"):
        findings.append(Finding(
            rule_id="FXPROMO-PROV-001", severity="blocker", scope="bundle",
            reason_code="identity.provider_id_mismatch",
            message=(
                f"fx-provider-extract provider_id {provider.get('provider_id')!r} does not match "
...
def check_manifest_authorization_findings(manifest: Mapping[str, Any], *, pair_id: str, as_of_date: str) -> list[Finding]:
    """Wrap :func:`fundamental_pipeline.valuation.market.registry.check_manifest_authorization`
    (lifecycle/scope/effective-window/temporary-use-exception) as
    findings, using the FX ``pair=`` authorization path."""
    problems = check_manifest_authorization(dict(manifest), pair=pair_id, as_of_date=as_of_date)
    return [
        Finding(
            rule_id="FXPROMO-AUTH-001", severity="blocker", scope="bundle",
...
def check_temporal_bounds(*, retrieved_at: str, cutoff_instant: str, effective_at: str) -> list[Finding]:
    ...
    if retrieved_at > cutoff_instant:
        findings.append(Finding(
            rule_id="FXPROMO-TIME-001", severity="blocker", scope="bundle",
            reason_code="temporal.retrieval_after_cutoff",
            message=f"fx-provider-extract retrieved_at {retrieved_at!r} is after cutoff_instant {cutoff_instant!r}",
        ))
    if effective_at > cutoff_instant:
        findings.append(Finding(
            rule_id="FXPROMO-TIME-002", severity="blocker", scope="bundle",
...
def check_effective_at_matches_trade_date(*, effective_at: str, trade_date: str) -> list[Finding]:
    if effective_at[:10] != trade_date:
        return [Finding(
            rule_id="FXPROMO-TIME-003", severity="blocker", scope="bundle",
...
def select_eligible_rows(
    ...
    *,
    ...
    """Filter normalized ``fx-rate-series`` rows down to the ones that are
    structurally eligible for promotion. Never uses ``adjusted_close``/
    ``open``/``high``/``low``. Never treats missing as zero."""
    ...
    for row in rows:
        ...
        if row.get("provider_symbol") != provider_symbol:
            continue
        if row.get("base_currency") != base_currency or row.get("quote_currency") != quote_currency:
            continue
...
def select_row(
    ...
    *,
...
) -> tuple[SelectedFxRow | None, list[Finding]]:
    ...
    if not eligible_rows:
        return None, [Finding(
            rule_id="FXPROMO-ROW-001", severity="blocker", scope="bundle",
            reason_code="selection.no_eligible_row",
            message="no eligible valid, non-quarantined, positive-close row was found for this pair/date range",
        )]
    ...
    if len(candidates) > 1:
        return None, [Finding(
            rule_id="FXPROMO-ROW-003", severity="blocker", scope="bundle",
    ...
    if lag_days > max_fallback_lag_days:
        return None, [Finding(
            rule_id="FXPROMO-ROW-002", severity="blocker", scope="bundle",
