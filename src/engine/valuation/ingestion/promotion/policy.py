"""Pure EOD-close promotion engine: given already-loaded/parsed bundle
documents, a market-source manifest, and a promotion policy, deterministically
select one eligible row and build the three promotion documents in memory.
...
relevant decision comes from the caller-supplied bundle/manifest/policy
...
from .models import PromotionOutcome, PromotionPolicy, SelectedRow
...
def _build_observation(
    ...
    *,
    ...
    instrument_id = manifest["instrument_identity"]["instrument_id"]
    ...
    observation_id = _hash_id("eod-close-promotion-observation", {"manifest_id": manifest_id, **identity_payload})
    revision_id = _hash_id("eod-close-promotion-revision", {**identity_payload, "value": selected.close_value})
    source_record_ref = (
    ...
    return {
        ...
        "instrument_or_pair": instrument_id,
...
def build_promotion(
    *,
    ...
    price_series_doc: Mapping[str, Any],
    manifest: Mapping[str, Any],
    ...
    findings += check_bundle_domain_eligibility(bundle_manifest_doc, ticker=ticker)
    findings += check_provider_manifest_compatibility(provider_extract_doc, manifest)
    findings += check_manifest_authorization_findings(manifest, ticker=ticker, as_of_date=as_of_date)
    ...
    expected_provider_symbol = (request.get("resolved_provider_symbols") or {}).get(ticker)
    expected_currency = (manifest.get("instrument_identity") or {}).get("trading_currency")
    start_date_inclusive = request.get("start_date_inclusive", "")
    end_date_exclusive = request.get("end_date_exclusive", "")

    selected: SelectedRow | None = None
    if not has_blocking(findings):
        eligible_rows = select_eligible_rows(
            price_series_doc.get("rows", ()),
    ...
    validation_doc = {
        ...
        "artifact_type": "eod_close_promotion_validation",
    ...
    promotion_id = _hash_id("eod-close-promotion", promotion_id_payload)

    promotion_manifest_doc = {
        "schema_version": "1.0.0",
        "artifact_type": "eod_close_promotion_manifest",
