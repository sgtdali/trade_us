...
everything else under ``engine.public``, never imports
``engine.valuation``. It also never reads committed
generated output (derived ratios, signals, summaries, reports) as a
FinancialBasis source -- only :mod:`engine.context`'s
already-validated authored-input loading (``AnalysisContext``) and the
period model in :mod:`engine.periods`.
...
#: engine.periods.Period.kind -> valuation-inputs schema's
...
class FinancialPeriodRef:
    ...
    identifier used to locate the period (``engine.periods``'
...
_NOT_AUTHORED = FieldValue(status="unavailable", reason_codes=("capital.field_not_authored_in_engine",))
...
def build_financial_basis(
    *,
    ...
    authored periods via :func:`engine.context.build_context`
    -> validate authored inputs (:func:`engine.validation.engine.validate_inputs`)
    ...
    only through ``engine.context.build_context``. Never
    ...
    basis = FinancialBasis(
        ...
        # parallel one.
        preferred_equity=_field_from_metric(direct_financials, "preferred_equity", missing_reason="capital.field_not_authored_in_engine"),
...
#: engine.registry.sector_modules today (confirmed: EREGL
