...
Attached: `pack.json`. It contains one company, VZ, plus its sector peer group. It was prepared by my own pipeline from SEC XBRL
...
| financial statement period end | 2026-06-30 |
...
For VZ the two prices differ by +0.5%. Price-based multiples are updated by the exact price ratio.
...
VZ has not filed since the cutoff, so its financials here are the latest ones.
