...
Attached: `pack.json`. It contains one company, RDDT. It was prepared by my own pipeline from SEC XBRL
...
For RDDT the two prices differ by -3.4%. Price-based multiples are updated by the exact price ratio.
...
RDDT has not filed since the cutoff, so its financials here are the latest ones.
...
Use company-tearsheet for RDDT.
