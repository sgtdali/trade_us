...
Attached: `pack.json`. It contains one company, GOOGL. It was prepared by my own pipeline from SEC XBRL
...
For GOOGL the two prices differ by -3.0%. Price-based multiples are updated by the exact price ratio.
...
GOOGL has not filed since the cutoff, so its financials here are the latest ones.
...
Use company-tearsheet for GOOGL.
