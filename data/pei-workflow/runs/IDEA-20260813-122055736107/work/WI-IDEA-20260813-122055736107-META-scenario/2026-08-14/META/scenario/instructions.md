...
Attached: `pack.json`. It contains one company, META. It was prepared by my own pipeline from SEC XBRL
...
For META the two prices differ by -2.2%. Price-based multiples are updated by the exact price ratio.
...
META has not filed since the cutoff, so its financials here are the latest ones.
...
Use scenario-sensitivity-generator for META.
