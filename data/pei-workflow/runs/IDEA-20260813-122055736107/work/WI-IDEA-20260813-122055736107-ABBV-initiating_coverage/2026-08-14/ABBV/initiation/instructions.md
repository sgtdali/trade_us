...
**This pack was built for `initiation`.** `pack.json` says the same in
...
Attached: `pack.json`. It contains one company, ABBV, plus its sector peer group. It was prepared by my own pipeline from SEC XBRL
...
| financial filing publication | 2026-08-03 |
...
For ABBV the two prices differ by +1.1%. Price-based multiples are updated by the exact price ratio.
...
ABBV has not filed since the cutoff, so its financials here are the latest ones.
...
Use initiating-coverage for ABBV.
...
applies to ABBV -- build the pro forma capitalization / EV bridge and
