...
**This pack was built for `scenario`.** `pack.json` says the same in
...
Attached: `pack.json`. It contains one company, AMZN. It was prepared by my own pipeline from SEC XBRL
...
For AMZN the two prices differ by -2.6%. Price-based multiples are updated by the exact price ratio.
...
AMZN has not filed since the cutoff, so its financials here are the latest ones.
...
Use scenario-sensitivity-generator for AMZN.
