...
Attached: `pack.json`. It contains one company, NFLX. It was prepared by my own pipeline from SEC XBRL
...
| financial filing publication | 2026-07-17 |
...
For NFLX the two prices differ by +0.1%. Price-based multiples are updated by the exact price ratio.
...
NFLX has not filed since the cutoff, so its financials here are the latest ones.
