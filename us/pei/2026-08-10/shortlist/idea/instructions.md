...
Attached: `pack.json`. It contains 18 US large-cap companies carried forward from a sector-level screen. The names that screen did not carry forward are not in this pack, so this is the first time these companies are being compared with each other. It was prepared by my own pipeline from SEC XBRL
...
2026-08-10. `price_reconciliation` shows both and the gap. 0 of 18 names differ by 5% or more, the widest by 1%.
...
Use idea-generation across all 18 companies in one screen.
...
End with a single table covering all 18 tickers, one row each:
